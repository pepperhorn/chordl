...
- **The same level control now filters piano voicings, not just guitar shapes (`@pepperhorn/chordl-voicings`, `@pepperhorn/chordl-react`).** Switching to Keyboard display used to leave the Beginner/Emerging/Established radio inert — every voicing from a plain triad to a rootless altered shell was offered regardless of the chosen level. `chordl-voicings` gains its own copy of the same three-rung ladder (`EXPERIENCE_LADDER`, `levelForVoicing`, `isCoreQuality`, `CORE_TEMPLATES`) and a selector, `selectVoicingsForExperience`, with the identical cumulative-matching, widen-one-rung-at-a-time contract as the guitar side — pinned equal by a `chordl-react` test so the two copies cannot silently drift. `VoicingVariantToggle` takes the new `level` prop and filters its A/B/C variants on it, showing a "no `<rung>` voicing — showing `<wider rung>` instead" notice when it has to widen rather than going blank; `dev/App.tsx`'s existing level radio now drives it.

  Widening is the common case here, and that is by design, not a shortfall: only 2 of the voicing library's 73 entries — `close-sus2` and a new two-note `power-5-shell` — carry every tone of a core triad/seventh/sus/power-chord shape in a hand-sized span. Every maj7, dom7 and sus4 entry in the library is a shell, rootless, drop or spread voicing that deliberately omits or displaces a core tone, so none of them qualifies as beginner even though those qualities are on the allowed list — a beginner still gets an easy root-position maj7/dom7/sus4 in the app, just from `generateVariants`' inversion path rather than from the library. The power chord (`5`) previously had no beginner voicing at all: its two pitch classes are 5 and 7 semitones apart, so any three-note spelling of it spans exactly 12 semitones, one past the beginner bound, no matter which octave the notes are picked from. `power-5-shell` drops the doubled root and plays the bare fifth instead. This is a behaviour change beyond the level filter: **`findVoicing("5")` now returns `[0, 7]` instead of `[0, 7, 12]`**, because the new shell entry is ordered first in the library and wins the existing first-match-per-style lookup — any consumer reading `findVoicing("5")`'s interval count directly should account for the shorter voicing.

  Also added, and worth flagging precisely because it introduces nothing visible: `spellingsFor(pitchClasses)` in `chordl-voicings`, which names every chord a pitch-class set spells (an augmented triad or diminished seventh genuinely has more than one correct name — no voicing resolves which). It is exported and tested but rendered nowhere in this change, so if you go looking for a spelling-ambiguity badge or warning in the UI, it isn't there yet; how often the ambiguity actually shows up in the corpus hasn't been measured, and that measurement should decide the presentation.

### Fixed

- **A library voicing is now drawn where it is declared, on `PianoChord`'s main note-based path (`@pepperhorn/chordl-voicings`, `@pepperhorn/chordl-core`, `@pepperhorn/chordl-react`).** Each entry in the voicing library places its notes with `intervals` — semitone offsets from the root, octaves included — and that placement *is* the voicing: it is what makes a Drop 2 a Drop 2 and a "tenth" shell a tenth. The renderer reduced an entry to bare pitch classes and rebuilt the octaves from the note letters, bumping an octave whenever a letter failed to rise, so the shape drawn was a function of spelling rather than of the declared intervals. Nine of the library's seventy-two entries disagreed with themselves in at least one key: the three "tenth" shells drew as plain thirds, and six four-note grips drew an octave or two wider than one hand can hold — `rootless-dom7-a` on C, for instance, drew E4 A4 Bb5 D6 where the entry declares E4 A4 Bb4 D5. New in `chordl-voicings`: `voicingOctaveOffsets(root, entry, octave?)`, index-parallel to `voicingPitchClasses`, plus an optional `octaveOffsets` on `VoicingVariant` (library variants only, and not consumed inside this repo — see its doc comment). `calculateLayout` (`chordl-core`) gains an `octaveOffsets` layout option so the visible keyboard is sized for the real span. `PianoChord` prefers the declared octaves for the keyboard highlights, the keyboard window and the staff alike, so the two views still cannot disagree. Inversions and algorithmic variants are untouched: they carry no octave information — their ordered pitch classes *are* the voicing — and keep the ascending-stack rule. Two other paths that place notes from letters alone are **not** touched by this fix and remain open bugs of the same kind: `PianoChord`'s slash-bass ("over X") branch, which returns before `calculateLayout` and derives placement from letter comparisons, and the progression renderer (`progression-resolver.ts` → `ChordGroup`), which calls `calculateLayout` with no offsets at all.
